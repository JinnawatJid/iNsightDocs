          ...
          <div class="step-body" v-if="step.completed || step.rejected">
            <div v-if="step.comment" class="comment-text-wrapper">
              <template v-for="(line, lIndex) in step.comment.split('\n')" :key="'text-'+lIndex">
                <div v-if="!line.startsWith('ปรับวงเงินจาก') && !line.startsWith('ปรับเครดิตเทอมจาก') && !line.startsWith('อนุมัติวงเงินที่') && !line.startsWith('อนุมัติเงื่อนไขเครดิตที่')" class="comment-text-line">{{ line }}</div>
              </template>
              <div v-if="step.comment.split('\n').some(l => l.startsWith('ปรับวงเงินจาก') || l.startsWith('ปรับเครดิตเทอมจาก') || l.startsWith('อนุมัติวงเงินที่') || l.startsWith('อนุมัติเงื่อนไขเครดิตที่'))" class="audit-trail-container">
                <template v-for="(line, lIndex) in step.comment.split('\n')" :key="'audit-'+lIndex">
                  <div v-if="line.startsWith('ปรับวงเงินจาก') || line.startsWith('ปรับเครดิตเทอมจาก') || line.startsWith('อนุมัติวงเงินที่') || line.startsWith('อนุมัติเงื่อนไขเครดิตที่')" class="audit-trail-line">
...
const WORKFLOW_STEPS = [
  ...
  { id: 'step-5', roleKeys: ['ผู้อนุมัติ (วงเงินต่ำกว่าเกณฑ์)', 'ผู้อนุมัติ (วงเงิน <300K)', 'ผู้จัดการฝ่ายการเงิน'], label: 'ผู้จัดการฝ่ายการเงิน' },
  { id: 'step-6', roleKeys: ['ผู้อนุมัติ (วงเงินสูงกว่าเกณฑ์)', 'ผู้อนุมัติ (วงเงิน > 300K)', 'กรรมการเครดิต', 'กรรมการเครดิต (Legacy)'], label: 'กรรมการเครดิต' }
];
