      ...
      <div class="unified-card">
        ...
        <div class="mt-4">
          <div
            ...
            @click="showConcurrencySettings = !showConcurrencySettings"
          >
        ...
        <tbody>
          ...
          >
            ...
            <td>
              ...
              <span v-else class="log-message" :title="item.log">{{
                item.log
              }}</span>
            </td>
          </tr>
          <tr v-if="queue.length === 0">
            <td colspan="11" class="text-center">
              ไม่มีข้อมูล กรุณาอัปโหลดไฟล์ Excel
            </td>
          </tr>
        </tbody>
      </table>
    ...
    >
      <div class="modal-content">
        ...
        <p class="modal-subtitle">
          อัปโหลดไฟล์เอกสารการเงินสำหรับ
          <b>{{ uploadTargetItem?.customerId }}</b>
        </p>
        ...
        <div class="modal-body">
          <div class="checkbox-group">
            <label class="checkbox-label">
              <input
                type="checkbox"
                v-model="uploadForm.isNoFinancialData"
                @change="handleCheckboxChange"
              />
              ลูกค้าไม่ส่งงบการเงิน
            </label>
            <small class="helper-text"
              >(ระบบจะข้ามการดึงข้อมูลจาก DBD
              และประเมินวงเงินใหม่โดยอ้างอิงจากยอดซื้อเท่านั้น)</small
            >
          </div>
          ...
          <div
            class="upload-fields"
            :class="{ 'fields-disabled': uploadForm.isNoFinancialData }"
          >
            <div class="form-group">
              ...
              <input
                ...
                class="form-control"
                @change="(e) => handleFileChange('profile', e)"
              />
            </div>
            <div class="form-group">
              ...
              <input
                ...
                accept=".xlsx"
                class="form-control"
                :disabled="uploadForm.isNoFinancialData"
                @change="(e) => handleFileChange('balanceSheet', e)"
              />
            </div>
            <div class="form-group">
              ...
              <input
                ...
                class="form-control"
                :disabled="uploadForm.isNoFinancialData"
                @change="(e) => handleFileChange('incomeStatement', e)"
              />
            </div>
            <div class="form-group">
              ...
              <input
                ...
                class="form-control"
                :disabled="uploadForm.isNoFinancialData"
                @change="(e) => handleFileChange('financialRatios', e)"
              />
            </div>
          </div>
        </div>
...
const getRowClass = (item) => {
  ...
  if (
    ...
    !item.isReady &&
    item.isCompany &&
    item.log.includes("รอโหลดไฟล์ DBD")
  )
    return "row-warning";
  ...
  try {
    ...
    for (const code of branchCodesToFetch) {
      try {
        ...
        errorMessages.push(
          `สาขา ${code}: ${err.response?.data?.error || err.message}`,
        );
      }
    }
    ...
    if (allData.length === 0) {
      ...
      Swal.fire(
        "สำเร็จ",
        successMsg,
        errorMessages.length > 0 ? "warning" : "success",
      );
    }
  } catch (error) {
...
const findBestIdColumn = (data) => {
  ...
  const candidates = headers.map((header) => {
    ...
    if (
      ...
      score += 20;
    else if (h.includes("id") || h.includes("no") || h.includes("code"))
      score += 10;
    ...
    if (uniquenessRatio === 1.0)
      score += 25; // Perfect uniqueness
    else if (uniquenessRatio >= 0.9) score += 15;
    else if (uniquenessRatio < 0.5)
      score -= 40; // High duplication -> Penalty (e.g. "AY", "AY", "AY")
    ...
    const isAlphanumeric = nonEmptySamples.every((s) =>
      /^[A-Z0-9\-_.]+$/i.test(s),
    );
    ...
    const avgLength =
      nonEmptySamples.reduce((a, b) => a + b.length, 0) /
      nonEmptySamples.length;
...
const showDebugFiles = async (item) => {
  ...
  if (!item.debugFiles && item.isCompany) {
    try {
      const checkRes = await axios.get(
        `/api/financials/check-local/${item.customerId}`,
      );
  ...
  if (item.taxId || item.dbdCompanyName) {
    htmlContent +=
      '<div style="background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 5px; padding: 10px; margin-bottom: 15px;">';
    ...
    if (item.name && item.dbdCompanyName) {
      ...
      if (
        normDbd &&
        normDyn &&
        !normDbd.includes(normDyn) &&
        !normDyn.includes(normDbd)
      ) {
  ...
  htmlContent +=
    '<p style="margin-bottom: 15px;">คลิกที่ปุ่มเพื่อดาวน์โหลดไฟล์ต้นฉบับ:</p>';

  if (item.isNoFinancialData) {
    htmlContent +=
      '<p style="color: #dc3545; font-weight: bold; margin-bottom: 15px;">⚠️ สถานะ: ลูกค้าไม่ส่งงบการเงิน</p>';
  }
...
const submitUpload = () => {
  ...
  if (!data.isNoFinancialData) {
    if (
      ...
      Swal.fire(
        "ข้อมูลไม่ครบ",
        'กรุณาอัปโหลดไฟล์ให้ครบ 4 ไฟล์ หรือ เลือก "ลูกค้าไม่ส่งงบการเงิน"',
        "warning",
      );
      return;
    }
...
const uploadLocalFiles = async (item, data) => {
  ...
  try {
    ...
    formData.append(
      "no_financial_data",
      data.isNoFinancialData ? "true" : "false",
    );
    ...
    if (data.incomeStatement)
      formData.append("profit_loss", data.incomeStatement);
    if (data.financialRatios)
      formData.append("financial_ratios", data.financialRatios);

    const res = await axios.post(
      `/api/financials/upload-local/${item.customerId}`,
      formData,
      {
    ...
    if (res.data.success) {
      ...
      item.log = data.isNoFinancialData
        ? "พร้อมดำเนินการ (ไม่ส่งงบฯ)"
        : "อัปโหลดไฟล์สำเร็จ พร้อมดำเนินการ";
...
const checkReadiness = async () => {
  ...
  try {
    ...
    const res = await axios.post("/api/financials/check-local-batch", {
    ...
    if (res.data.success) {
      ...
      const readyItems = results.filter(
        (r) => r.isReady && !r.isSkipped && !r.noFinancialData,
      );
      ...
      queue.value.forEach((item) => {
        ...
        if (checkRes) {
          ...
          item.isNoFinancialData =
            checkRes.noFinancialData || checkRes.isNoFinancialData || false;
          ...
          if (checkRes.isSkipped && item.status === "Pending") {
            ...
            item.log = item.isNoFinancialData
              ? `พร้อมดำเนินการ (ไม่ส่งงบฯ)`
              : `มีไฟล์พร้อมดำเนินการ`;
            if (!item.isNoFinancialData) {
              item.debugFiles = {
                profile: {
                  type: "local",
      ...
      Swal.fire({
        ...
        if (
          ...
          const exportData = notReadyItems.map((nr) => {
            const originalItem = queue.value.find(
              (q) => q.customerId === nr.customerId,
            );
            return {
    ...
    Swal.fire(
      "ข้อผิดพลาด",
      "ไม่สามารถตรวจสอบไฟล์ได้: " + error.message,
      "error",
    );
  }
};
  ...
  while (!shouldStop.value) {
    ...
    try {
      ...
      if (customer.financial_summary?.total_purchase_3_months) {
        const rawVal = String(
          customer.financial_summary.total_purchase_3_months,
        ).replace(/,/g, "");
      ...
      if (!skipDBD) {
        ...
        // Step 1: Check if financial files (DBD Profile) already exist locally on the server.
        try {
          ...
          if (localCheck && localCheck.exists) {
            ...
            item.isNoFinancialData = localCheck.isNoFinancialData || false;
            item.log = item.isNoFinancialData
              ? "ใช้ข้อมูลที่มีอยู่ (ไม่ส่งงบฯ)"
              : "ใช้ข้อมูลที่มีอยู่ (Local)";
      ...
      if (analyzeRes.data.success) {
        ...
        if (analyzeRes.data.finalInputs) {
          if (analyzeRes.data.finalInputs.registeredCapital > 0) {
            item.registeredCapital =
              analyzeRes.data.finalInputs.registeredCapital;
          }
        ...
        if (
          analyzeRes.data.financialSummary?.latePaymentData
            ?.average_late_days !== undefined
        ) {
          item.latePaymentAverage =
            analyzeRes.data.financialSummary.latePaymentData.average_late_days;
        }
...
/**
 * Initiates the batch processing loop.
 * Orchestrates the queue and kicks off the bridging/download polling
 * process for each customer sequentially.
 */
const startBatch = async () => {
  ...
  const sourceText =
    inputType.value === "branch"
      ? `สาขา ${selectedBranch.value}`
      : "ไฟล์ Excel";
  const modelText =
    selectedModel.value === "new" ? "ลูกค้าใหม่" : "ลูกค้าปัจจุบัน";
  ...
  if (!isBridgeReady) {
    Swal.fire(
      "ข้อผิดพลาด",
      "ไม่สามารถเชื่อมต่อกับ Local Bridge กรุณาตรวจสอบการตั้งค่า",
      "error",
    );
    return;
  }
...
const extractFinancialData = (item, key, prop = "displayValue") => {
  if (
    !item.analysisResult ||
    !item.analysisResult.scoringResult ||
    !item.analysisResult.scoringResult.breakdown
  ) {
    return prop === "score" ? 0 : "-";
  }
  ...
  for (const g of groups) {
    ...
    if (breakdown && breakdown.items) {
      ...
      if (found) {
        ...
        if (prop === "value")
          return found.value !== undefined ? found.value : found.displayValue;
...
const exportFullDetailReport = () => {
  ...
  const validItem = queue.value.find(
    (item) =>
  ...
  if (validItem) {
    const validHistory =
      validItem.analysisResult.financialSummary.monthlyHistory.slice(1, 7);
  ...
  const data = queue.value.map((item) => {
    const branchCode =
      item.customerId && item.customerId.length > 2
    ...
    if (
      item.analysisResult &&
      item.analysisResult.financialSummary &&
      item.analysisResult.financialSummary.monthlyHistory
    ) {
    ...
    const total3Actual = history3.reduce(
    ...
    const row = {
      ...
      คะแนนระยะเวลาธุรกิจ: extractFinancialData(
        item,
        "years_in_business",
        "score",
      ),

      สัดส่วนเครดิตที่ขอต่อทุนจดทะเบียน: extractFinancialData(
        item,
        "leverage",
        "value",
      ),
      "คะแนน สัดส่วนเครดิตที่ขอต่อทุนจดทะเบียน": extractFinancialData(
        item,
        "leverage",
        "score",
      ),
      กรรมสิทธิ์ทรัพย์สิน: extractFinancialData(
        item,
        "asset_ownership",
        "value",
      ),
      "คะแนน กรรมสิทธิ์ทรัพย์สิน": extractFinancialData(
        item,
        "asset_ownership",
        "score",
      ),
      ...
      "อัตราการส่วนหนี้สินรวม ต่อส่วนของผู้ถือหุ้น": extractFinancialData(
        item,
        "de_ratio",
        "value",
      ),
      "คะแนน อัตราการส่วนหนี้สินรวม ต่อส่วนของผู้ถือหุุ้น":
        extractFinancialData(item, "de_ratio", "score"),
      อัตราการหมุนเวียนของสินค้าคงเหลือ: extractFinancialData(
        item,
        "inventory_turnover",
        "value",
      ),
      "คะแนน อัตราการหมุนเวียนของสินค้าคงเหลือ": extractFinancialData(
        item,
        "inventory_turnover",
        "score",
      ),
      "ความสามารถในการชำระหนี้ (DSCR)": extractFinancialData(
        item,
        "dscr",
        "value",
      ),
      "คะแนน อัตราส่วนความสามารถในการชำระหนี้ (DSCR)": extractFinancialData(
        item,
        "dscr",
        "score",
      ),
      ...
      สัดส่วนรายได้ต่อทุนจดทะเบียน: extractFinancialData(
        item,
        "revenue_capital_ratio",
        "value",
      ),
      "คะแนน สัดส่วนรายได้ ต่อทุนจดทะเบียน": extractFinancialData(
        item,
        "revenue_capital_ratio",
        "score",
      ),

      "สัดส่วนยอดซื้อเฉลี่ย ย้อนหลัง 3 เดือนต่อเครดิตที่ขอ":
        currentLimit > 0 ? total3 / 3 / currentLimit : 0,
      "สัดส่วนยอดซื้อเฉลี่ย ย้อนหลัง 6 เดือนต่อเครดิตที่ขอ":
        currentLimit > 0 ? total6 / 6 / currentLimit : 0,
      "คะแนน สัดส่วนยอดซื้อเฉลี่ย ย้อนหลัง 6 เดือน ต่อเครดิตที่ขอ":
        extractFinancialData(item, "capacity_check", "score"),

      สัดส่วนยอดซื้อต่อระยะเวลาเครดิตที่ขอ: extractFinancialData(
        item,
        "turnover_speed",
        "value",
      ),
      "สัดส่วนยอดซื้อต่อระยะเวลาเครดิตที่ขอ (เฉลี่ย 6 เดือน)":
        currentLimit > 0
          ? total6 / 6 / currentLimit
          : extractFinancialData(item, "turnover_speed", "value"),
      "คะแนน ยอดซื้อต่อระยะเวลาเครดิตที่ขอ": extractFinancialData(
        item,
        "turnover_speed",
        "score",
      ),
      ...
      "คะแนน แนวโน้มการซื้อ": extractFinancialData(
        item,
        "purchase_trend",
        "score",
      ),
      ระยะเวลาของการเป็นลูกค้า: extractFinancialData(
        item,
        "customer_duration",
        "value",
      ),
      "คะแนน ระยะเวลาการเป็นลูกค้า": extractFinancialData(
        item,
        "customer_duration",
        "score",
      ),

      "เฉลี่ยการจ่ายเงินล่าช้า (WADL)":
        item.wadlScore !== null ? item.wadlScore : 0,
      ...
      ระยะเวลาเครดิตรวมวางบิล:
        (Number(item.paymentTerms) || 0) +
        getBillingDurationValue(item.billingTerms),
    };
