...
* **Multi-folder File Resolution:** Financial Excel files are stored in date-based subfolders (`customers/[customerNo]/[date_folder]/`). Because different document types might be uploaded on different days, the `getCustomerFinancialData` function does *not* restrict its search to only the single newest date folder. Instead, it iterates through all date folders from newest to oldest, resolving the path for each required Excel file (`DBD_BalanceSheet.xlsx`, etc.) independently. This prevents "Missing File" errors when files exist but are spread across multiple historic uploads.
