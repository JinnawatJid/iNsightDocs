...
- **Directories:** Ensures `downloads/` directory exists within `release/backend/`. User uploads and OCR temp processing files are externalized through `UPLOAD_PATH=../../uploads`, while persistent customer DBD storage uses `CUSTOMERS_PATH=../../customers`.
- **.env Template:** Generates a template `.env` file populated with default production database connection strings (e.g., `DB_SERVER=localhost`, `DB_NAME=CreditRequestDB`) and configures both `UPLOAD_PATH=../../uploads` and `CUSTOMERS_PATH=../../customers`. Maintainers must update this file on the target server.
