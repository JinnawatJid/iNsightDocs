...
2. **การจัดการระหว่างสร้างคำขอ:** ในขณะที่สร้างคำขอ ระบบบังคับให้ต้อง `แนบไฟล์เอกสาร` (`<<includes>>`) ให้ครบถ้วนตามประเภทลูกค้า และมีทางเลือกให้ผู้ใช้สามารถพักการทำงานโดย `บันทึกฉบับร่าง` (`<<extends>>`) ได้
3. **การส่งคำขอ:** เมื่อกรอกข้อมูลเสร็จสิ้นและผู้ใช้กด `ส่งคำขอเครดิต` ระบบจะบังคับ `ตรวจสอบความครบถ้วน` (`<<includes>>`) ก่อนที่จะอนุญาตให้ส่งเข้าระบบ
