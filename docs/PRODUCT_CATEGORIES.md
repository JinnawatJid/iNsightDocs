...
-   **Body:** `{"customer_code": "...", "months": 3 | 6}`
    -   `months`: Dynamically passed as `6` if the customer's current credit limit is > 10 (existing customer), or `3` if the limit is ≤ 10 (new customer without credit limit). Note that this is strictly used for the category calculation, whereas the top-level "Total Purchases" widget always evaluates against 3 months.
