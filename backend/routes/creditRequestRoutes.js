...
// The POST '/' route is a unified endpoint for both creating new drafts
// and updating existing transactions (e.g., status approvals).
...
router.get(
  "/:id/files/:fileId",
  creditRequestController.downloadCreditRequestFile,
);
...
router.post(
  ...
  upload.any(),
  creditRequestController.uploadAdditionalDocument,
);
router.delete(
  "/:id/files/:fileId",
  creditRequestController.deleteAdditionalDocument,
);
